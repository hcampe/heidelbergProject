                ...
                for a in self.active_agents:
                    if (not a.dead) and (a.x, a.y) in explosion.blast_coords:
                        ...
                        if a is explosion.owner:
                            ...
                            self.logger.info(f'Agent <{explosion.owner.name}> receives 5 points')
...
class BombeRLeWorld(GenericWorld):
    ...
    def build_arena(self):
        ...
        arena[:, :1] = WALL
        arena[:, -1:] = WALL
        if self.args.scenario == "challenge-coin-collector":
            arena[int(s.COLS/2+3),2:s.ROWS-2] = WALL
        else:
        ...
        if self.args.scenario == "challenge-coin-collector":
            start_position = (s.COLS//2 + 2, s.ROWS//2+1)
        else:
        ...
        if self.args.scenario == "challenge-coin-collector":
            coins = [Coin((s.COLS//2+4,s.ROWS//2), collectable=True)]
        else:
            coins = []
        ...
        for exp in self.explosions:
            if exp.is_dangerous():
                for (x, y) in exp.blast_coords:
                    explosion_map[x, y] = max(explosion_map[x, y], exp.timer - 1) #-1 weil wenn exp.timer 1 dann im nächsten schritt schon ungefährlich also 0 eintrag
